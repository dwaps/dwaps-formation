...
--
-- Client :  localhost
-- Généré le :  Mer 25 Janvier 2017 à 17:48
...
(36, 25, 'Création d\'une base pour les tests', 'Je vous propose de cr&eacute;er une base pour un site social de recettes de cuisine. Pour avoir le droit de poster une recette, chaque utilisateur devra &ecirc;tre authentifi&eacute; (il y aura donc une table <em>users</em> qui devra poss&eacute;der un champ <em>role</em>). Une fois connect&eacute;, l\'internaute aura acc&egrave;s &agrave; un tableau de bord et il pourra cr&eacute;er autant de recettes qu\'il le souhaite. D\'autre part, chaque recette pourra &ecirc;tre comment&eacute;e par les membres du site.<br /><br />Voici sans tarder le code SQL que je vous propose :<br /><code style="display: block; width: 90%; background: #000; color: white; margin-top: 10px; margin-bottom: 20px; padding-top: 5px; padding-bottom: 5px; padding-left: 5px;">CREATE DATABASE IF NOT EXISTS cooking_dwaps;<br />USE cooking_dwaps;<br /><br />CREATE TABLE user (<br />&nbsp;&nbsp; id&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; INT NOT NULL AUTO_INCREMENT,<br />&nbsp;&nbsp; name&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; VARCHAR(100) NOT NULL,<br />&nbsp;&nbsp; surname&nbsp;&nbsp;&nbsp; VARCHAR(100),<br />&nbsp;&nbsp; email&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; VARCHAR(255) NOT NULL,<br />&nbsp;&nbsp; password&nbsp;&nbsp; VARCHAR(8) NOT NULL,<br />&nbsp;&nbsp; role&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; VARCHAR(50),<br />&nbsp;&nbsp; image&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; VARCHAR(255),<br />&nbsp;&nbsp; created_at DATETIME,<br />&nbsp;&nbsp; <br />&nbsp;&nbsp; CONSTRAINT pk_user PRIMARY KEY(id),<br />&nbsp;&nbsp; CONSTRAINT uc_user UNIQUE (id, email)<br />) ENGINE = INNODB CHARSET = UTF8;<br /><br />CREATE TABLE recipe (<br />&nbsp;&nbsp; id&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; INT NOT NULL AUTO_INCREMENT UNIQUE,<br />&nbsp;&nbsp; user_id&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; INT NOT NULL,<br />&nbsp;&nbsp; name&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; VARCHAR(100),<br />&nbsp;&nbsp; content &nbsp; &nbsp;&nbsp; text,<br />&nbsp;&nbsp; category&nbsp;&nbsp;&nbsp;&nbsp; VARCHAR(100) NOT NULL,<br />&nbsp;&nbsp; image&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; VARCHAR(255),<br />&nbsp;&nbsp; created_at&nbsp;&nbsp; DATETIME,<br />&nbsp;&nbsp; updated_at&nbsp;&nbsp; DATETIME,<br />&nbsp;&nbsp; status&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; VARCHAR(30),<br />&nbsp; <br />&nbsp;&nbsp; CONSTRAINT pk_recipe PRIMARY KEY(id),<br />&nbsp;&nbsp; CONSTRAINT fk_recipe_user FOREIGN KEY(user_id) REFERENCES user(id)<br />) ENGINE = INNODB CHARSET = UTF8;<br /><br />CREATE TABLE comment (<br />&nbsp;&nbsp; id&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; INT NOT NULL AUTO_INCREMENT UNIQUE,<br />&nbsp;&nbsp; user_id&nbsp;&nbsp;&nbsp; INT NOT NULL,<br />&nbsp;&nbsp; recipe_id&nbsp; INT NOT NULL,<br />&nbsp;&nbsp; content&nbsp;&nbsp;&nbsp; text,<br />&nbsp;&nbsp; created_at DATETIME,<br />&nbsp;<br />&nbsp;&nbsp; CONSTRAINT pk_comment PRIMARY KEY(id),<br />&nbsp;&nbsp; # Un commentaire appartient &agrave; un utilisateur...<br />&nbsp;&nbsp; CONSTRAINT fk_comment_user FOREIGN KEY(user_id) REFERENCES user(id),<br />&nbsp;&nbsp; # ... et concerne une recette<br />&nbsp;&nbsp; CONSTRAINT fk_comment_recipe FOREIGN KEY(recipe_id) REFERENCES recipe(id)<br />) ENGINE = INNODB CHARSET = UTF8;<br /></code>\r\n<p>&nbsp;</p>', 1),
(37, 25, 'Création du bundle des entités', 'Nous allons cr&eacute;er un bundle sp&eacute;cialis&eacute; dans la gestion des donn&eacute;es du projet. J\'ai d&eacute;cid&eacute; de l\'appeler DWAPSModelBundle mais libre &agrave; vous bien s&ucirc;r de le nommer comme vous voulez. Petit rappel seulement, en Symfony il est pr&eacute;f&eacute;rable de nommer ses bundle ainsi : NomDeLEntrepriseRoleDuBundle. Mais vous &ecirc;tes compl&egrave;tement libre de faire comme bon vous semble. Sachez toutefois que si vous d&eacute;sirez partager votre module, c\'est plus sympa si vous respecter cette convention.<br /><br />C\'est parti :<br /><code style="display: block; width: 90%; background: #000; color: white; margin-top: 10px; margin-bottom: 20px; padding-top: 5px; padding-bottom: 5px; padding-left: 5px;">cd monSuperProjet<br />php bin/console generate:bundle<br /></code>\r\n<p><code style="display: block; width: 90%; background: #000; color: white; margin-top: 10px; margin-bottom: 20px; padding-top: 5px; padding-bottom: 5px; padding-left: 5px;">Bundle name: DWAPS/ModelBundle<br />Bundle name [DWAPSModelBundle]:<br />Target Directory [src/]:<br />Configuration format (annotation, yml, xml, php) [annotation]:<br /></code></p>\r\n<p>&nbsp;Voil&agrave;, vous pouvez laisser la configuration par d&eacute;faut, &ccedil;a ne nous importe pas dans ce tuto. Je r&eacute;capitule donc : nous venons de cr&eacute;er un bundle pour la gestion mod&egrave;le du projet Symfony, le r&eacute;pertoire d\'installaiton est src/ (dans mon cas &ccedil;a donne src/DWAPS/ModelBundle/), les routes sont r&eacute;f&eacute;renc&eacute;es directement dans le(s) controlleur(s) du bundle (via des annotations).<br /><br />Voici l\'arborescence r&eacute;sultante :<br /><code style="display: block; width: 90%; background: #000; color: white; margin-top: 10px; margin-bottom: 20px; padding-top: 5px; padding-bottom: 5px; padding-left: 5px;">src/<br />&rarr; DWAPS/<br />&rarr; &rarr; ModelBundle/<br />&rarr; &rarr; &rarr; Controller/<br />&rarr; &rarr; &rarr; &rarr; DefaultController.php<br />&rarr; &rarr; &rarr; Resources/<br />&rarr; &rarr; &rarr; &rarr; config/<br />&rarr; &rarr; &rarr; &rarr; &rarr; services.yml<br />&rarr; &rarr; &rarr; &rarr; views/<br />&rarr; &rarr; &rarr; &rarr; &rarr; Default/<br />&rarr; &rarr; &rarr; &rarr; &rarr; &rarr; index.html.twig<br /></code></p>\r\n<p>&nbsp;Comme notre bundle servira juste &agrave; la gestion du model et n\'affichera aucune vue, vous pouvez d&eacute;lester le fichier app/config/routing.yml. Supprimez donc all&egrave;grement les lignes concernant votre bundle :<br /><code style="display: block; width: 90%; background: #000; color: white; margin-top: 10px; margin-bottom: 20px; padding-top: 5px; padding-bottom: 5px; padding-left: 5px;">dwaps_model:<br />&nbsp; resource:&nbsp; "@DWAPSModelBundle/Controller/"<br />&nbsp; type:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; annotation<br />&nbsp; prefixe:&nbsp;&nbsp; /</code> </p>', 1),
(38, 25, 'Import de la base et création des entités', 'Pour pouvoir importer la base de donn&eacute;es suscr&eacute;&eacute;e, il faut la d&eacute;clarer &agrave; Symfony. Pour cela, &eacute;ditez le fichier app/config/parameters.yml et &eacute;crivez-y les infos de connections &agrave; la base :<br /><code style="display: block; width: 90%; background: #000; color: white; margin-top: 10px; margin-bottom: 20px; padding-top: 5px; padding-bottom: 5px; padding-left: 5px;">parameters:<br />&nbsp;&nbsp; database_host: 127.0.0.1<br />&nbsp;&nbsp; database_port: null<br />&nbsp;&nbsp; database_name: cooking_dwaps<br />&nbsp;&nbsp; database_user: root<br />&nbsp;&nbsp; database_password: ########<br />&nbsp;&nbsp; ...<br /></code>\r\n<p>&nbsp;On peut maintenant se connecter sans probl&egrave;me &agrave; notre BDD et demander &agrave;&nbsp;<a href="http://www.doctrine-project.org/" target="_blank">Doctrine</a> de g&eacute;n&eacute;rer pour nous les entit&eacute;s directement en php. Pour cela, il faut indiquer dans la commande le format de sortie <em>annotation</em> :</p>\r\n<code style="display: block; width: 90%; background: #000; color: white; margin-top: 10px; margin-bottom: 20px; padding-top: 5px; padding-bottom: 5px; padding-left: 5px;">php bin/console doctrine:mapping:import DWAPSModelBundle annotation</code>\r\n<p>Comme vous pouvez le voir, il suffit de donner &agrave; Doctrine le nom de notre bundle pour qu\'il trouve tout seul dans quel r&eacute;pertoire op&eacute;rer. Remarque : si vous ne sp&eacute;cifier pas de format de sortie, Doctrine ira cr&eacute;er un fichier de configuration <em>XML</em> dans VotreBundle/Resources/config/doctrine/. Personnellement, je n\'affectionne pas trop ce format. Si j\'ai besoin de sortir un fichier de configuration pour mes entit&eacute;s afin d\'&eacute;viter de les noyer dans les annotations, je pr&eacute;f&egrave;re le <em>yml</em> que je trouve beaucoup plus lisible. Vous pouvez essayer les commandes suivantes et voir ce que &ccedil;a donne si vous le voulez :<br /><code style="display: block; width: 90%; background: #000; color: white; margin-top: 10px; margin-bottom: 20px; padding-top: 5px; padding-bottom: 5px; padding-left: 5px;">php bin/console doctrine:mapping:import DWAPSModelBundle<br />php bin/console doctrine:mapping:import DWAPSModelBundle php<br />php bin/console doctrine:mapping:import DWAPSModelBundle yml<br /> </code></p>\r\n<p>Autre chose importante : parce que j\'ai choisi le format <em>annotation</em>, mes entit&eacute;s ont &eacute;t&eacute; g&eacute;n&eacute;r&eacute;es et je n\'ai pas besoin de faire autre chose. Par contre si vous avez choisi de mapper vos entit&eacute;s (en utilisant un des formats autoris&eacute;s : <em>yml</em>, <em>php</em> ou <em>xml</em>), vous devrez effectuer une commande suppl&eacute;mentaire pour les g&eacute;n&eacute;rer :<br /><code style="display: block; width: 90%; background: #000; color: white; margin-top: 10px; margin-bottom: 20px; padding-top: 5px; padding-bottom: 5px; padding-left: 5px;">php bin/console doctrine:generate:entities DWAPSModelBundle<br /></code></p>\r\n<p>&nbsp;Vos entit&eacute;s se trouvent maintenant bien au chaud dans leur r&eacute;pertoire <em>Entity</em>, pr&ecirc;tes &agrave; &ecirc;tre utilis&eacute;e. Enfin... presque pr&ecirc;tes ! Si vous avez fait comme moi et que vous avez choisi le format <em>annotation</em>, il ne faut surtout pas oubli&eacute; de g&eacute;n&eacute;rer les <em>getters</em> et les <em>setters</em>, &eacute;videmment. Le probl&egrave;me, c\'est que je n\'ai pas mapp&eacute; les entit&eacute;s et qu\'elles ne sont donc pas enregistr&eacute;es dans les m&eacute;tadonn&eacute;s de Doctrine. En clair, Doctrine ne pourra pas g&eacute;n&eacute;rer les fonctions dont j\'ai besoin pour l\'exploitation de mes objets. Mais il y a une solution heureusement : <br /><code style="display: block; width: 90%; background: #000; color: white; margin-top: 10px; margin-bottom: 20px; padding-top: 5px; padding-bottom: 5px; padding-left: 5px;">php bin/console doctrine:mapping:info<br />[Exception]<br />You don\'t have any mapped Doctrine ORM entities according to the current configuration. Bla bla et encore bla bla...<br /><br /># SOLUTION :<br />php bin/console doctrine:mapping:import DWAPSModelBundle yml<br /></code></p>\r\n<p>Et l&agrave; vous devriez normalement me sauter dessus : "pourquoi &ecirc;tre passer par les annotations si de toute fa&ccedil;on il fallait un fichier de mappage ?". Et vous avez tout &agrave; fait raison ! Cependant, j\'ai une excuse : c\'&eacute;tait pour faire l\'erreur avec vous afin de mettre le doigt sur la solution. De cette fa&ccedil;on, je vous &eacute;vite du temps perdu &agrave; rechercher une solution par vous-m&ecirc;me. Bref, j\'esp&egrave;re que vous comprenez &agrave; quel point j\'ai voulu vous rendre service... <br /><br />A pr&eacute;sent vous pouvez faire comme vos camarades :<br /><code style="display: block; width: 90%; background: #000; color: white; margin-top: 10px; margin-bottom: 20px; padding-top: 5px; padding-bottom: 5px; padding-left: 5px;">php bin/console doctrine:generate:entities DWAPSModelBundle:Comment<br />php bin/console doctrine:generate:entities DWAPSModelBundle:Recipe<br />php bin/console doctrine:generate:entities DWAPSModelBundle:User<br /><br />Ou encore mieux, en une seule commande :<br /><br />php bin/console doctrine:generate:entities DWAPSModelBundle<br /></code></p>\r\n<p>Avec cette commande pas besoin d\'&eacute;crire soi-m&ecirc;me les accesseurs et mutateurs ! Elle est pas belle la vie !</p>', 1);
...
INSERT INTO `dwaps_tuto` (`id`, `image_id`, `title`, `description`, `date`, `slug`) VALUES
(15, 15, 'Intégrer un éditeur de texte WYSIWYG', 'Si vous avez besoin de concevoir un back-office, l\'interface d\'administration d\'un site web par exemple, ou encore un simple blog, votre client vous sera très reconnaissant d\'avoir pensez à lui faciliter la vie en intégrant une solution d\'édition de texte dans votre projet...', '2017-01-17 16:31:21', 'integrer-un-editeur-de-texte-wysiwyg'),
(17, 17, 'Prise en main du logiciel Git', 'Dans ce tuto, vous apprendrez à prendre en main ce logiciel de contrôle de version très populaire et facilitant l\'avancement de projet autant que le travail en équipe.', '2017-01-18 09:47:37', 'prise-en-main-du-logiciel-git'),
(18, 18, 'Création d\'un menu avec animate.css', 'Animate.css fait partie de ces petits frameworks bien utiles qu\'il faut toujours avoir à portée de main. Il permet d\'intégrer facilement des animation CSS dans votre projet.', '2017-01-18 12:24:18', 'creation-d-un-menu-avec-animate-css'),
(19, 19, 'Création d\'une app de base avec Ionic 2', 'La différence entre la 1ère et la 2ème version de Ionic est extraordinaire. Cela est dû au passage d\'AngularJS à Angular 2 d\'une part et de la montée en puissance des Web components. Dans ce tuto, je vous propose une prise en main de ce framework.', '2017-01-18 12:54:37', 'creation-d-une-app-de-base-avec-ionic-2'),
(20, 20, 'Les flexbox : pour une mise en page impeccable', 'Ce modèle de boîte flexible vous permettra de créer des layouts adaptatifs efficaces et faciles à mettre en place. Si vous êtes web designer et que vous ne connaissez pas encore cet outil, vous ne le regretterez pas !', '2017-01-18 14:39:01', 'les-flexbox-pour-une-mise-en-page-impeccable'),
(21, 21, 'Reproduire le logo DWAPS en CSS', 'Vous ne le savez peut-être pas mais avec du pur CSS, on peut en faire des choses ! Y compris dessiner... Je vous propose ici de reproduire le logo DWAPS de toute pièce uniquement avec ce langage. Une fois fini et pour le rendre un peu plus vivant, on lui affectera une animation que nous coderons également en CSS.', '2017-01-18 15:01:03', 'reproduire-le-logo-dwaps-en-css'),
(22, 22, 'La gestion de dépendance avec Bower', 'A mesure qu\'un projet web grossit, il devient difficile de gérer ses librairies. Sans parler des librairies dont dépendent ces librairies... Bref, le casse-tête est de taille. Bower est justement une solution à ce problème : c\'est un gestionnaire de paquet côté front-end que je vous propose de découvrir sans plus attendre.', '2017-01-18 15:37:15', 'la-gestion-de-dependance-avec-bower'),
(23, 23, 'Découverte de Bootstrap', 'Vous en avez assez de réécrire du code CSS rébarbatif  pour la gestion de l\'adaptabilité multi écran par exemple ? Utilisez Bootstrap ! Ce framework vous permettra de concevoir plus rapidement votre projet en responsive web design. Avec lui, il n\'y a même plus besoin de se préoccuper de la création d\'une feuille de style de base ou d\'un reset CSS !', '2017-01-18 16:12:30', 'decouverte-de-bootstrap'),
(24, 24, 'Mettre en place un serveur web', 'Je vous propose ici de découvrir comment installer et configurer un serveur Apache 2 sur Linux. Cet outil est indispensable pour tester ses sites Internet avant leur hébergements pour le public. Avec une gestion multi-thread en plus d\'une gestion multi-processus, ce serveur gère les requêtes clients sans difficultés, vous ne serez pas déçu.', '2017-01-18 16:30:37', 'mettre-en-place-un-serveur-web'),
(25, 25, 'Créer des entités à partir de la BDD', 'Dans un projet Symfony, si le client ne me fournit pas de base, je crée mes entités avant de les insérer en base de données car cela m\'évite d\'avoir à coder en SQL. En effet, Symfony met à notre disposition tout un panel de lignes de commandes dont certaines sont spécifiques à Doctrine et rendent la tâche plus aisée. Dans le cas d\'une base existante, il est nécessaire de faire le chemin inverse : c\'est ce que nous verrons dans ce tuto...', '2017-01-18 22:04:38', 'creer-des-entites-a-partir-de-la-bdd'),
(26, 26, 'Une commande pour un environnement', 'Dans ce tuto, je vous partage un petit truc que j\'ai l\'habitude de faire pour me décharger d\'ouvrir toutes les applications dont j\'ai besoin pour travailler quand je suis sous Linux : un petit script shell sympa. Je le lance systématiquement à chaque démarrage de mon ordinateur, et il me rend bien service...', '2017-01-19 18:01:48', 'une-commande-pour-un-environnement');
...
(1, 'dwaps', 'dwaps', 'contact@dwaps.fr', 'contact@dwaps.fr', 1, NULL, '$2y$13$UXO9t59AOz7AGwkti38sf.PqwqEVnuaBExJrrUFGZGkY5jJdwDXdW', '2017-01-25 17:05:49', NULL, NULL, 'a:1:{i:0;s:16:"ROLE_SUPER_ADMIN";}');
